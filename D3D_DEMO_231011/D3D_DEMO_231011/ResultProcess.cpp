...
void ResultProcess::Init(float posX, float posY, float size, float interval)
{
	...
	for (auto& stepnum : StepNum) {
		...
		//モデル作成
		stepnum->CreateModel(g_Assets->Result_num, m_size * 74.83f, m_size * 110.5f, 10, 1);

		stepnum->InitAnimation(false);

		stepnum->m_anime->isPlaying = false;
...
void ResultProcess::UpdateCharCenter(char* outputChar)
{
	...
	float totalWidth = totalCharacters * (74.f / SCREEN_PARA / 2);
	...
	for (int i = 0; i < totalCharacters; i++)
	{
		...
		// 文字の位置を設定する（中央に調整）
		float posX = m_sPosX - totalWidth / 2 + i * (74.0f/ SCREEN_PARA / 2) + m_interval;
		
		StepNum[i]->InitPos(posX, m_sPosY, 0.1f);
...
{	
	for (int i = 0; i < strlen(outputChar); i++)
	{
		...
		StepNum[i]->InitPos(m_sPosX + posX, m_sPosY, 0.1f);

		//UVOFFSET更新
		StepNum[i]->m_anime->Update();

		if (Game::Get()->GetFade()->m_fadePanel->m_materialDiffuse.w <= 0.01f) {
		//描画
		StepNum[i]->Draw();
		}
	}
