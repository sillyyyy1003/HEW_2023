...
ID3D11BlendState* GetBlendMultiply(void) { return g_pD3D->BlendMultiply; }

void SetBlendState(ID3D11BlendState* _blendState)
{
    g_pD3D->Context->OMSetBlendState(_blendState, NULL, 0xffffffff);
}
...
BOOL D3D_CreateBlendState(void)
{
    ...
    BlendState.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
    hr = g_pD3D->Device->CreateBlendState(&BlendState, &(g_pD3D->BlendMultiply));
    if (FAILED(hr)) {
        return FALSE;
        //エラー表示
        MessageBoxA(NULL, "BlendStateAddictive Failed", "エラー", MB_OK | MB_ICONERROR);
